...
export default function MemoryContinuityIntelligence(){
  const [data,setData] = useState(null);

  useEffect(()=>{
    fetch("/api/memory-continuity-intelligence/dashboard")
      .then(r=>r.json())
      .then(setData)
      .catch(()=>setData(null));
  },[]);
  ...
  const map = data?.continuityMap || [];

  return(
    <main style={theme.page}>
      <section style={theme.shell}>
        <h1 style={{fontSize:48,color:theme.gold}}>
          Brian & Co Memory & Continuity Intelligence
        </h1>

        {data && (
          <>
            <section style={theme.card}>
              <h2 style={{color:theme.gold}}>Continuity Status</h2>
              <p>{seed.status}</p>
              <p>{seed.protectedPhases}</p>
            </section>

            <section style={{marginTop:24,...theme.card}}>
              <h2 style={{color:theme.gold}}>Continuity Areas</h2>
              {seed.continuityAreas.map(item=><p key={item}>◆ {item}</p>)}
            </section>
            ...
            <section style={{marginTop:24,...theme.card}}>
              <h2 style={{color:theme.gold}}>Protected Continuity Standard</h2>
              <p>
                This phase prepares continuity intelligence only. It does not edit products,
