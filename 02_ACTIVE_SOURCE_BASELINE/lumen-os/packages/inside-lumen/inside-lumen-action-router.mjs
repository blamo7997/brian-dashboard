...
export async function routeInsideLumenAction({
  ...
  try {
    ...
    } else if (clean.includes("native everything") || clean.includes("sdk") || clean.includes("all products") || clean.includes("manufacturer") || clean.includes("backend") || clean.includes("frontend") || clean.includes("cia security") || clean.includes("self change")) {
      action = "native-everything";
      const nativeMod = await import("../native-everything-fabric/native-everything-fabric-engine.mjs");
      const productMod = await import("../universal-product-monitor/universal-product-monitor-engine.mjs");
      const serviceMod = await import("../lumen-service-abstraction/lumen-service-abstraction-engine.mjs");
      const securityMod = await import("../cia-plus-security/cia-plus-security-engine.mjs");
      result = {
        native: nativeMod.nativeEverythingStatus({ reason: "inside-lumen-command" }),
        products: productMod.universalProductMonitorStatus({ reason: "inside-lumen-command" }),
        services: serviceMod.serviceAbstractionStatus({ reason: "inside-lumen-command" }),
        security: securityMod.ciaPlusSecurityStatus({ reason: "inside-lumen-command" })
      };
