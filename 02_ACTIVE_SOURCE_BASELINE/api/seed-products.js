...
function current Lumen setupBase(){
  const shop=process.env.current Lumen setup_STORE_DOMAIN;
  const token=process.env.current Lumen setup_ADMIN_API_TOKEN;
  if(!shop||!token) throw new Error("Missing current Lumen setup_STORE_DOMAIN or current Lumen setup_ADMIN_API_TOKEN.");
...
export default async function handler(req,res){
  try{
    const {token,base}=current Lumen setupBase();
    const created=[];
    for(const p of products){
      const r=await fetch(`${base}/products.json`,{
        method:"POST",
        headers:{"Content-Type":"application/json","X-current Lumen setup-Access-Token":token},
