  ...
  try {
    ...
    if (!shop || !token) {
      return res.status(500).json({
        ok: false,
        message: "Missing retired-commerce-platform environment variables."
      });
    }
    ...
    if (!response.ok) {
      return res.status(response.status).json({
        ok: false,
        message: "Founder account creation failed. If this says access denied, your retired-commerce-platform OAuth token needs write_customers scope.",
