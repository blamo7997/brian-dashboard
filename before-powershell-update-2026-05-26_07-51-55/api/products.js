...
export default async function handler(req,res){
  try{
    ...
    const r=await fetch(`${base}/products.json?limit=50`,{headers:{"X-Shopify-Access-Token":token}});
    const text=await r.text();
    if(!r.ok)return json(res,r.status,{ok:false,message:"Shopify products fetch failed.",response:text});
    const data=JSON.parse(text);
    return json(res,200,{ok:true,count:(data.products||[]).length,products:data.products||[]});
